...
# 실행 이력(스냅샷) 테이블. 스테이징은 조회가 끝나면 곧 사라지는 임시본이라,
# "이 실행에서 어떤 값이 나왔는지"라는 흔적은 별도로 보관해야 클라우드에서
# 앱이 재시작돼도(파일시스템이 껑껑이라 outputs/ 로컬 폴더가 날아가도) 과거
# 실행 기록이 남는다.
RUN_HISTORY_TABLE = "run_history"

# run_history의 만료를 매 실행마다 "지금부터 이만큼 뒤"로 되돌린다(진짜
# 만료 없음이 아니다 — 실측으로 확인함: 결제 계정 없는 BigQuery 샌드박스는
# "60일 미만"을 강제해서, expires=None(무기한)으로 두려는 API 호출 자체가
# 403으로 거부된다). 59일로 잡아 매 실행이 만료를 60일 한도 안에서 계속
# 미래로 밀어내는 식으로 굴린다 — 파이프라인이 이 기간 안에 한 번이라도
# 돌면 이력은 사실상 끊기지 않는다. 완전한 무기한 보관은 결제 계정을 붙여야
# 가능하다.
RUN_HISTORY_TTL_DAYS = 59
